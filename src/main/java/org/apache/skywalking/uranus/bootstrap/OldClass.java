...
public class OldClass extends BaseBootstrap {
    ...
    public static void parseOptions(String[] args, boolean old) {
        ...
        String oldArtifacts = commandLine.getOptionValue("c");
        ...
        if (old) {
            classList = MavenUtils.getOldClassList(oldArtifactList, newArtifactList);
        } else {
            classList = MavenUtils.getNewClassList(oldArtifactList, newArtifactList);
        }
